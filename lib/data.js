...
// Pure search over ANY product list (mock or Sanity). Used server-side by
// lib/catalog.js and client-side by SearchClient / CatalogExplorer.
export function filterProductsByQuery(list, query, { brands: brandList = brands, categories: categoryList = categories } = {}) {
  const q = (query || '').trim().toLowerCase();
  if (!q) return [];
  const brandName = (slug, p) => p.brandName || (brandList.find((b) => b.slug === slug) || {}).name || slug;
  const categoryName = (slug, p) => p.categoryName || (categoryList.find((c) => c.slug === slug) || {}).name || slug;
  return (list || []).filter((p) => {
    const haystack = [
      p.name, p.brand, brandName(p.brand, p), p.category, categoryName(p.category, p),
...
// Simple client-side search across many fields (demo data).
